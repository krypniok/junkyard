...
#ifdef __linux__
void sendWindowToBackground(Window x11Window) {
    char wmctrlCmd[256];
    snprintf(wmctrlCmd, sizeof(wmctrlCmd), "wmctrl -i -r %lu -b add,below", x11Window);
    system(wmctrlCmd);
}
#endif
...
unsigned long getWindowIdFromXprop(Window x11Window) {
    char xpropCmd[256];
    snprintf(xpropCmd, sizeof(xpropCmd), "xprop -id %lu _NET_WM_WINDOW_ID | awk '{print $3}'", x11Window);

    FILE* fp = popen(xpropCmd, "r");
    if (fp == NULL) {
        fprintf(stderr, "Failed to execute xprop command\n");
        return 0;
    }

    unsigned long windowId = 0;
    fscanf(fp, "0x%lx", &windowId);
    pclose(fp);
...
void getX11RootWindowSize(Display* display, int* width, int* height) {
    Window root;
    int x, y;
    unsigned int border, depth;

    XGetGeometry(display, DefaultRootWindow(display), &root, &x, &y, (unsigned int*)width, (unsigned int*)height, &border, &depth);
}
...
int main(int argc, char* argv[]) {
    ...
    getX11RootWindowSize(display, &rootWidth, &rootHeight);

    window = SDL_CreateWindow("Animated Sprite Game", 0, 0, rootWidth, rootHeight, SDL_WINDOW_OPENGL
    | SDL_WINDOW_SHOWN | SDL_WINDOW_BORDERLESS | SDL_WINDOW_SKIP_TASKBAR); //| SDL_WINDOW_FULLSCREEN_DESKTOP);
    if (!window) {
        exit(1);
    }
    ...
    if (SDL_GetWindowWMInfo(window, &wmInfo)) {
        ...
        // Print the X11 window ID
        printf("X11 Window ID: %lu\n", x11Window);
    }
#endif
sendWindowToBackground(x11Window);

    // start cinnamon ?
    char command[1000];
    char outputFile[] = "/dev/null";

    sprintf(command, "export DISPLAY=:1; cinnamon-session &");
