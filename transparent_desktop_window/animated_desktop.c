...
GLuint generateTexture(int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            unsigned char r = (quadX + quadY) % 2 == 0 ? 0 : 255;
            unsigned char g = (quadX + quadY) % 2 == 0 ? 0 : 255;
            unsigned char b = (quadX + quadY) % 2 == 0 ? 0 : 255;
