...
/* read MISCELLANEOUS reg */
	*regs = port_byte_in(VGA_MISC_READ);
	regs++;
/* read SEQUENCER regs */
	for(i = 0; i < VGA_NUM_SEQ_REGS; i++)
	{
		port_byte_out(VGA_SEQ_INDEX, i);
		*regs = port_byte_in(VGA_SEQ_DATA);
		regs++;
	}
/* read CRTC regs */
	for(i = 0; i < VGA_NUM_CRTC_REGS; i++)
	{
		...
		*regs = port_byte_in(VGA_CRTC_DATA);
		regs++;
	}
